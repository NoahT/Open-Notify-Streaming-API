...
class ISSLocationFirestoreClientTestSuite(unittest.TestCase):
  ...
  def setUpClass(self) -> None:
    # Same database; I really just didn't want to move out of free tier. In the future we
    # might consider installing the Firestore emulator and ITs with Docker compose so
    # we can run the integration tests in a more isolated manner.
  ...
  def test_should_get_iss_location_documents_correctly(self) -> None:
    ...
    ts_to = int(time.time())
    ts_from = ts_to - 10
    # Rough window. Could fail due to concurrent runs. Again, can be fixed in future if
    # emulator inside container is used for integration tests.
