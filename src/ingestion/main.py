...
client = KazooZookeeperClient(kazoo_client=KazooClient(hosts=os.getenv('ZOO_SERVERS'), logger=logging.getLogger(__name__)))
client.start()
election_facade = SequentialEphemeralElectionFacade(zookeeper_client=client, logger=logging.getLogger(__name__))
